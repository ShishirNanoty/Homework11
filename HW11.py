...
try:
    while True:
        # print('Enter done! to exit\nEnter C to create new database')
        key1 = input('Enter done! to exit\nEnter C to create new database\n')
        if key1 == 'done!':
            break
        elif key1 == 'C':
            print('Enter following details to create database:')
            make = input('Enter make of vehicle: ')
            model = input('Enter model of vehicle: ')
            year = int(input('Enter year of manufacturing of vehicle: '))
            weight = int(input('Enter weight of vehicle: '))
            v = Cars(make, model, year, weight)
            v_list.append(v)
        else:
            raise ValueError()
